...
WHERE CardID = @cardId AND IDXe = @idXe AND IDMat = @idMat;";
        ...
        {
            // Giữ nguyên các kiểm tra kết nối
            if (connection == null || connection.State != ConnectionState.Open)
            {
                MessageBox.Show("Chưa kết nối với cơ sở dữ liệu. Vui lòng kết nối trước..", "Cảnh báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
...
WHERE GiaTien > 0 AND "; // Giữ nguyên điều kiện GiaTien > 0
        ...
        AS DATETIME)
            ...
            // Giữ nguyên khối try-catch và logic đổ dữ liệu vào dgvResults
            try
            {
                using (SqlCommand command = new SqlCommand(query, connection))
                {
                    ...
                    using (SqlDataAdapter adapter = new SqlDataAdapter(command))
                    {
                        ...
                        txtSum.Text = totalGiaTien.ToString("N0") + " VNĐ";
                        txtCount.Text = dataTable.Rows.Count.ToString("N0");
                        btnExportRevenue.Enabled = true; // Giữ nguyên dòng này
                    }
                }
            }
            catch (Exception ex)
...
";
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            {
                DialogResult result = passwordForm.ShowDialog(); // Hiển thị hộp thoại và chờ kết quả
